...
def grouped_gemm_compute_offs(group_lens: torch.Tensor) -> torch.Tensor:
    return torch.ops.group_gemm_primus_cpp_extension.grouped_gemm_compute_offs(group_lens)
...
def grouped_gemm_csrc_impl(
    ...
    out = torch.ops.group_gemm_primus_cpp_extension.grouped_gemm(
...
def grouped_gemm_variable_k_csrc_impl(
    ...
    out = torch.ops.group_gemm_primus_cpp_extension.grouped_gemm_variable_k(
        a, b, group_lens, group_offs, trans_a, trans_b, num_cu
    )
    return out
